...
**Presets data model & Era toggle:** Chainring/cassette options are **not** in HTML — they live in the `PRESETS` object at the top of `app.js` (`PRESETS[component][set]`, where `component` is `chainring|cassette` and `set` is `modern|vintage|single`; each is a list of `{ group, options:[[label,value]] }`). `PRESETS` is pure data with **no `document` references** so it is node-testable. The 4 preset `<select>`s in `index.html` ship empty; `buildPresetOptions(select, groups)` renders their `<optgroup>`/`<option>`s and appends the `Custom...` entry (Custom is never stored in `PRESETS`). Each preset `<select>` has its **own** 3-way Era toggle — front (chainring) and rear (cassette) eras are independent per drivetrain, so there are **4** toggles: `#era-chainring-a`, `#era-cassette-a`, `#era-chainring-b`, `#era-cassette-b`. `applyEra(suffix, component)` repopulates just that one select from `PRESETS[component][era]`, resets to the first option, and recomputes; `getEra(suffix, component)` reads the active button of the matching toggle. This exists because `<select>` allows only **one** `<optgroup>` nesting level — the Era toggle substitutes for the missing second level (Modern/Vintage/Single can't be nested groups). Decoupling front/rear lets you mix sets, e.g. a Modern chainring with a Vintage 7-speed cassette. Load defaults are applied via the `LOAD_DEFAULTS` map after population. Wheel and tire `<option>`s are **not** part of this model — they stay hardcoded in `index.html` (not era-filtered); a `BMX / FGFS` tire optgroup intentionally repeats numeric widths that also exist under `MTB` for discoverability.
...
**Toggle class is shared — scope by data attribute:** the speed-unit toggle and all four Era toggles use `.toggle`/`.toggle-btn`. JS must disambiguate: unit buttons carry `data-unit`, era buttons `data-era`. `getSpeedUnit()` queries `.toggle-btn[data-unit].active`; the unit handler clears only `[data-unit]`; each Era toggle is scoped to its own `#era-<component>-<suffix>` container. A global `.toggle-btn` selector would cross-wire them.

**Drivetrain duplication:** All A/B *input elements* are duplicated in HTML with `-a`/`-b` id suffixes (but option lists are not — see Presets data model). The "← copy A" button (`copy-a-to-b`) mirrors every A input into B; it copies A's **two eras first** (chainring + cassette independently), calls `applyEra('b', component)` for each to rebuild B's option lists, then mirrors the specific selections and `.visible` custom-field state.
